...
public class SeparableEnemySolver {
    ...
    Graph g;
    public HashMap<String, String> edgeTo;
    ...
    SeparableEnemySolver(String filename) throws java.io.FileNotFoundException {
        ...
        edgeTo = new HashMap<>(g.labels().size());
        marked = new HashSet<>(g.labels().size());
    }
    ...
    private boolean notSeparable(String v) {
        int n = 1;
        String w = edgeTo.get(v);
        while (!w.equals(v)) {
            w = edgeTo.get(w);
            n++;
        }
    ...
    private void isSeparableHelper(String v) {
        if (!isSeparable) {
            return;
        }
        marked.add(v);
        for (String w : g.neighbors(v)) {
            if (marked.contains(w) && !edgeTo.get(w).equals(v)) {
                edgeTo.put(v, w);
                if (notSeparable(v)) {
                    isSeparable = false;
                }
